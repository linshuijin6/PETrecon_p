...
def window_partition(x, window_size, input_resolution, window_size_plus=1):
    """
    Args:
        x: (B, H, W, C)
    ...
    """
    window_size = [window_size_plus, window_size] if input_resolution[1]==4 else [window_size, window_size_plus]
    B, H, W, C = x.shape
...
def window_reverse(windows, window_size, H, W, window_size_plus=1):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
    ...
    Returns:
        x: (B, H, W, C)
    """
    B = int(windows.shape[0] / (H * W / window_size_plus / window_size))
    x = windows.view(B, H // window_size_plus, W // window_size, window_size_plus, window_size, -1) if W // window_size else windows.view(B, H // window_size, W // window_size_plus, window_size, window_size_plus, -1)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., input_size=(1, 45), window_size_plus=1):

        super().__init__()
        self.dim = dim
        self.window_size = [window_size_plus, window_size] if input_size[1]==4 else [window_size, window_size_plus] # Wh, Ww
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., window_size_plus=1,
        ...
        self.window_size_plus = window_size_plus
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, input_size=self.input_resolution, window_size_plus=self.window_size_plus)
    ...
    def calculate_mask(self, x_size):
        ...
        window_size_plus = self.window_size_plus
    ...
    def forward(self, x, x_size):
        ...
        x_windows = window_partition(shifted_x, self.window_size, self.input_resolution, self.window_size_plus)  # (576,8,8,180) (nW*B, window_size, window_size, C)  nW:number of Windows
        x_windows = x_windows.view(-1, self.window_size_plus * self.window_size, C)  # (576,64,180) (nW*B, window_size*window_size, C)  nW:number of Windows
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size_plus, self.window_size, C) if self.input_resolution[1]<128 else attn_windows.view(-1, self.window_size, 1, C)# (576,8,8,180) (nW*B, window_size, window_size, C)  nW:number of Windows
        shifted_x = window_reverse(attn_windows, self.window_size, H, W, self.window_size_plus)  # B H' W' C shifted_x (4,96,96,180) (batch_in_each_GPU, embedding_channel, H, W)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, window_size_plus=1):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, window_size_plus=window_size_plus)
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 img_size=224, patch_size=4, resi_connection='1conv', window_size_plus=1):
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 ...
                 use_checkpoint=False, upscale=1, img_range=1., upsampler='', resi_connection='1conv', window_size_plus=1,
                 **kwargs):
